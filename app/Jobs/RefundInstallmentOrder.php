...
class RefundInstallmentOrder implements ShouldQueue
{
    ...
    public function handle()
    {
        ...
        if (!$installment = Installment::query()->where('order_id', $this->order->id)->forst()) {
            return;
        }
        ...
        foreach ($installment->items as $item) {
            ...
            try {
                ...
                //假如某个还款计划还款保存了,则暂时跳过,继续处理下一个还款计划还款
                continue;
            }
        }
