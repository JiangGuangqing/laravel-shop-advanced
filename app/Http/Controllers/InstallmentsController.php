...
class InstallmentsController extends Controller
{
    ...
    public function payByAlipay(Installment $installment)
    {
        ...
        if (!$nextItem = $installment->items()->whereNull('paid_at')->orderBy('sequence')->first()) {
            //如果没有未支付的还款,原则上不可能,因为如果分期已经结清在上一个判断就退出了
            throw new InvalidRequestException('该分期订单已结清');
        }
        ...
        return app('alipay')->web([
            //支付订单号使用分期流水号+还款计划编号
            'out_trade_no' => $installment->no . '-' . $nextItem->sequence,
            'total_amount' => $nextItem->total,
            'subject' => '支付 laravel shop 的分期订单:' . $installment->no,
            //这里的 notify_url 和 return _url 可以覆盖掉在 AppServiceProvider 设置的回调地址
    ...
    public function alipayNotify()
    {
        ...
        //如果订单状态不是整个或者结束,则不走后续的逻辑
        if (!in_array($data->trade_status, ['TRADE_SUCCESS', 'TRADE_FINISHED'])) {
            return app('alipay')->success();
        }
        //拉起支付时使用的订单号是有分期流水号+还款计划编号组成的
        //因此可以通过支付订单号来还原出这笔还款是由哪个分期付款的哪个还款计划
        ...
        $item = InstallmentItem::query()
            ->whereHas('installment', function ($query) use ($no) {
                $query->whereHas('order', function ($query) use ($no) {
                    $query->where('refund_no', $no); //根据订单表退款流水号找到哦啊对应还款计划
                });
            })
            ->where('sequence', $sequence)
            ->first();

        //没有找到订单,原则上不可能发生,保证代码健壮性
        if (!$item) {
            return $failXml;
        }
